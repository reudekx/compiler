...
/*
                ...
                1. std::string 대신 더 효율적인 자료형 이용하여 문자열 처리하기
   (해결)
                ...
                4. Lexeme 조립 시 memcpy 등을 이용해 더욱 효율적으로 처리하기
   (해결)

                논의 사항

                1. 토큰으로 정의되지 않은 문자가 등장할 경우 처리 방안
                                1. 그대로 반환한다. (가령, 소괄호나 쉼표를
   파서에서 처리)
                                2. 처리가 필요한 모든 문자를 토큰으로 정의하고,
   정의되지 않은 문자는 그냥 무시하기
...
/*
                문자를 사용하고난 뒤 comsume을 호출 하여 다음 문자를 Char에 담을
   수 있다. 또한 Fc는 현재 Char의 index를 가리킨다.
