...
1. 현재 lookahead 토큰을 2개 유지하는데 프로그램이 1개 이하의 토큰으로 이뤄져
있을 때도 잘 처리되어야 함.
*/
...
void checkTerminal() {
  ...
  if (!Enum::match(TopSymbol, {type_tok, Token})) {
    std::cout << "Terminal = " << EnumString[TopSymbol.Type][TopSymbol.Value]
              << ", Token = " << EnumString[type_tok][Token] << std::endl;
    ParseState = tok_err;
    return;
  }
...
void emitBytecode() {
  std::cout << "==================== Emit Bytecode: " << TopSymbol.Value << " "
            << EnumString[TopSymbol.Type][TopSymbol.Value] << std::endl;
...
void parseBlockTail() {
  ...
  ParseStack.push(SB(top_expr));
  ParseStack.push(
      BC(pop)); // block 내의 문장의 사이에서 기존 문장의 결과값을 pop
}
...
void parseExprTail() {
  if (Enum::match(ParseStack.top(),
                  {type_tok, Token})) // 현재 토큰과, 파싱 스택의 TOP이 일치하는
                                      // 경우 -> 더 이상 확장시킬 필요가 없음.
  {
    return;
  }
  ...
  if (OperatorTable[op].LeftAssociative) {
    ...
    while (OperatorStack.top() != tok_sentinel) {
      if (Precedence <= OperatorTable[OperatorStack.top()].Precedence) {
        ...
        std::cout
            << "==================== Emit Bytecode: "
            << OperatorTable[OperatorStack.top()].Bytecode << " "
            << EnumString[type_bc][OperatorTable[OperatorStack.top()].Bytecode]
            << std::endl;
        OperatorStack.pop();
      } else {
        break;
...
void parseBinary() {
  pushOperator(Token);
  consume();
  // 아직 연산자별 처리를 가정하지 않았다. -> 연산자 테이블로 각각 처리하도록
  // 함. -> 실제 동작은 바이트코드 함수로 구현하면 됨.
}
...
void parseIfExpr() {
  ...
  ParseStack.push(BC(jump_if_zero)); // 조건 거짓일 시 다음 조건으로 점프 (POP은
                                     // 내장되어 있다.)
...
void parseElifExpr() {
  ...
  if (Token == tok_else) // 무조건 ok가 등장하므로, 여기서 처리
  {
    ...
    ParseStack.push(
        BC(jump)); // IF나 ELIF를 마치고 왔다면, 종료 부분으로 JUMP해야 한다.
    ParseStack.push(SB(push_jump_index));
    return;
  }
  if (Token == tok_elif) {
    ...
    ParseStack.push(SB(push_jump_index));
    ParseStack.push(SB(top_expr));
    ParseStack.push(SB(push_jump_target)); // 이전 IF 혹은 ELIF 조건 불만족 시
                                           // 여기로 JUMP해야 한다.
    ParseStack.push(
        BC(jump)); // IF나 ELIF를 마치고 왔다면, 종료 부분으로 JUMP해야 한다.
    ParseStack.push(SB(push_jump_index));
...
void parseForExpr() {
  ...
  ParseStack.push(
      SB(parameter)); // 반복시 적절히 다음 원소를 호출하도록 변경 필요
  ParseStack.push(SB(push_jump_target)); // 시작 위치
}
...
const std::vector<int> &parse() {
  while (!ParseStack.empty()) {
    ...
    ParseStack.pop();
    std::cout << TopSymbol.Value << " "
              << EnumString[TopSymbol.Type][TopSymbol.Value] << " / " << Token
              << " " << EnumString[type_tok][Token] << std::endl;
